...
'''
very similar to train_model.py, but allows to loop through different axis combinations for the acceleroemter signals
'''
...
def main():
    ...
    arg('--axis', type=str , help='accelerometer axes to use e.g. xy to use and x and y axis')
    args = vars(parser.parse_args())
    print(args)

    sig_type_source = []
    if 'x' in args['axis']:
        sig_type_source.append('aX')
    if 'y' in args['axis']:
        sig_type_source.append('aY')
    if 'z' in args['axis']:
        sig_type_source.append('aZ')
