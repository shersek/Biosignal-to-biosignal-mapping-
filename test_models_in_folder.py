...
config_testing = {
             ...
             'stride_testing': int(4.096*F_SAMPLING_TESTING )//2 ,
             'batch_size':32
}
...
#various results
fig=plt.figure()
plt.scatter([u[-20:-15]+u[-6::] for u in list_model_names] , [np.median(np.array(u)) for u in list_models_pearson ] )
...
plt.scatter([u[-20:-15]+u[-6::] for u in list_model_names], [np.median(np.array(u)) for u in list_y ]  )
